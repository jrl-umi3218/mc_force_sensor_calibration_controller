...
: mc_control::fsm::Controller(rm, dt, patchConfig(*rm, config))
{
  ...
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("This controller requires at least an encoder observer for robot {}. In addition, floating base robots also require estimation of the floating base orientation.", robot().name());
  }
